...
class CombatManager {
  ...
  static useAbility(player, habilidadeId, monsterManager, now) {
    ...
    const result = monsterManager.takeDamage(damageInfo.damage, player.nivel);
    return { ok: true, type: 'ability', playerId: player.id, playerName: player.nome, classeId: player.classeId, habilidadeId: ability.id, habilidadeNome: ability.nome, visual: ability.visual, cooldown: ability.cooldown, damage: result.damageApplied, isCrit: damageInfo.isCrit, monsterDied: result.died, result };
  }
