...
class CombatManager {
  ...
  static calculateDamage(player, ability) {
    ...
    const base = (stats.baseDano + gearDamage + player.nivel * stats.multiplicadorNivel) * (1 + (petBonus.ataquePct || 0) + (petBonus.danoPct || 0) + (talentBonus.ataquePct || 0) + (v30Bonus.ataquePct || 0) + (v40Bonus.ataquePct || 0) + (v90Bonus.ataquePct || 0) + metaAtk);
    const multiplier = ability ? ability.danoMultiplicador : 1;
    const critChance = Math.min(95, (stats.critico || 0) + gearCrit + (petBonus.critico || 0) + (talentBonus.critico || 0) + (v30Bonus.critico || 0) + (v40Bonus.critico || 0) + (v90Bonus.critico || 0) + ((ability && ability.bonusCritico) || 0));
  ...
  static grantKillRewards(player, combatEvent) {
    ...
    { const meta = MetaProgressManager.publicMeta(player); const zoneXp=(meta.zone&&meta.zone.bonus&&meta.zone.bonus.xpPct)||0; const metaXp=((meta.ascension&&meta.ascension.bonuses&&meta.ascension.bonuses.xpPct)||0)+((meta.ascension&&meta.ascension.artifactBonuses&&meta.ascension.artifactBonuses.xpPct)||0); return LevelManager.addXP(player, Math.floor(combatEvent.result.xpReward * (1 + (TalentManager.getBonuses(player).xpPct || 0) + (V30ProgressManager.getBonuses(player).xpPct || 0) + (V40ProgressManager.getBonuses(player).xpPct || 0) + (V90ProgressManager.getBonuses(player).xpPct || 0) + zoneXp + metaXp))); }
  }
}
